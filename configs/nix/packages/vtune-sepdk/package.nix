...
let
  ...
  version_majmin = builtins.head (
    builtins.match "([0-9]+\.[0-9]+)\.[0-9]+" intel-vtune.version
  );
in
stdenv.mkDerivation {
  pname = "vtune-sepdk";
  version = "${intel-vtune.version}-${kernel.version}";

  inherit (intel-vtune) src;
  ...
  unpackPhase = ''
    runHook preUnpack
    7za x $src _installdir/vtune/${version_majmin}/sepdk
    cd _installdir/vtune/${version_majmin}/sepdk/src
    runHook postUnpack
  '';
