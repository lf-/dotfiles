...
// OBJC-FIX: go_list.bzl has no channel for Objective-C sources -- it reads
// GoFiles, CgoFiles, HFiles, CFiles, CXXFiles, SFiles and SysoFiles, and
// upstream pkg_analyzer simply drops build.Package.MFiles on the floor, so a
// package's .m files are never compiled and its symbols go missing at link
// time. Reporting them as CFiles routes them through cgo_builder into
// cxx_compile_srcs, which dispatches on file extension: a .m source lands on
// the toolchain's objc_compiler_info and is compiled as Objective-C, which is
// what we want. Nothing downstream assumes CFiles are C specifically.
//
// matchlock needs this for github.com/Code-Hex/vz/v3, whose
// Virtualization.framework bindings are a dozen .m files.
func cFiles(pkg *build.Package) []string {
	if len(pkg.MFiles) == 0 {
		return pkg.CFiles
	}
	files := make([]string, 0, len(pkg.CFiles)+len(pkg.MFiles))
